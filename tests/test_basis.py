...
@given(
    integers(min_value=0, max_value=conf.harmonic_oscillator_max_n),
    reasonable_floats,
...
def test_overlap_1d_harmonic_oscillators_is_orthonormal():
    ho0 = HarmonicOscillator(n=0, center=0.0)
    ho1 = HarmonicOscillator(n=1, center=0.0)
    assert isclose(Overlap()(ho0, ho0), 1.0)
    assert isclose(Overlap()(ho0, ho1), 0.0)
...
def test_total_energy_integral_of_harmonic_oscillator():
    ...
    for n in range(10):
        ...
        assert isclose(pe + ke, ho0.energy)
...
def test_eigen_basis_on_diagonals():
    ...
    assert np.allclose(expected_vecs, eigb.eigvecs, atol=conf.small_number)
    assert all(isclose(e1, e2) for e1, e2 in zip(expected_energies, eigb.energies))
...
def test_time_evolution_of_stationary_state():
    ...
    assert np.allclose(initial_density, d1, atol=conf.small_number)
    assert np.allclose(initial_density, d2, atol=conf.small_number)
